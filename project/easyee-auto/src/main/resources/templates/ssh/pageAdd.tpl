	...
	<form id="${ClassName}AddForm" method="post" data-options="novalidate:true" <#if addMutilpart>enctype="multipart/form-data" action="${ClassName}/save"</#if>>
			...
			<#if addMutilpart><input name="rows" id="${ClassName}AddDialogPageSize" type="hidden"/></#if>
			 <table cellpadding="5">
				...
				<tr>
	    			<td>${info[2]}:</td>
	    			<td><input class="${info[7]} easyui-validatebox" name="${ClassName?uncap_first}.${info[1]}" <#if info[3]=="true">required="true"</#if> data-options="prompt:'input ${info[2]}...'"><#if info[3]=="true"><span class="required">*</span></#if></td>
	    		</tr>
				</#if>
				</#list>
				</#if>
...
<div id="${ClassName}Add-dlg-buttons">
	<a class="easyui-linkbutton" id="${ClassName}AddSaveBtn" iconCls="icon-ok" onclick="${ClassName}.add()"><s:text name="label.add"></s:text></a> 
	<a class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:uiEx.closeDialog('#${ClassName}AddDialog')"><s:text name="label.cancel"></s:text></a>
</div>